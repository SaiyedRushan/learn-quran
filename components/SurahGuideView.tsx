...
export default function SurahGuideView({
  guide,
  verses,
}: {
  guide: SurahGuide;
  ...
  return (
    <>
      {guide.reviewStatus === "draft" && (
        <div className="review-banner">
          ...
          <div className="rb-text">
            <strong>Draft — pending scholarly review.</strong> The Arabic and
            translation below are from verified sources, but the commentary
            (overview, memory hooks, vocabulary notes, recitation guidance) is an
            AI-assisted draft and has not yet been checked by a qualified scholar.
            Verify any point of ruling with a trusted teacher.
          </div>
        </div>
      )}
      ...
      <div className="tabs">
        <button
          className={`tab ${tab === "sections" ? "active" : ""}`}
        ...
        <button
          className={`tab ${tab === "recitation" ? "active" : ""}`}
              ...
              {isOpen && (
                <div className="sec-body">
                  ...
                  <div className="verses">
                    {sec.groups.map((g, gi) => renderGroup(g, gi))}
                  </div>
                  {sec.notes
                    .filter((n) => n.kind !== "core")
                    .map((n, ni) => {
      ...
      <div className={`panel ${tab === "recitation" ? "active" : ""}`}>
        {guide.recitation.intro && (
          <div className="rec-card" style={{ background: "#0d2e24", borderColor: "#1D9E75" }}>
            ...
            <Html
              className="hb-text"
              html={guide.recitation.intro.text}
            />
          </div>
        )}
        ...
        {guide.recitation.stops.length > 0 && (
          <>
            ...
            <div className="sub-label">
              {guide.recitation.stopsLabel || "Natural stopping points"}
            </div>
